...
const CACHE_DB_NAME = "VideoCacheDB"
const CACHE_STORE_NAME = "VideoStore"
...
export async function storeVideoInCache(videoId: string, videoUrl: string): Promise<void> {
  const db = await getDB()
  const cacheSize = await getCacheSize(db)

  while (cacheSize + videoUrl.length > MAX_CACHE_SIZE) {
...
export async function getVideoFromCache(videoId: string): Promise<string | undefined> {
  const db = await getDB()
  const result = await db.get(CACHE_STORE_NAME, videoId)

  if (result) {
    // Update LRU timestamp
    await updateCacheIndex(db, videoId)
