...
export function buildCreateRoute(
  request: Partial<UnifiedRequest> & { model: AnyModel; type: "image" | "video" },
  options?: { noCreateModal?: boolean },
): RouteLocationRaw {
  if (request.type === "image") {
    ...
    if (options?.noCreateModal) query.noCreateModal = "1"
...
export function toCreatePage(
  ...
  void router.push(buildCreateRoute(request, options))
}
